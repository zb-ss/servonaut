...
single-use token its preview issued for exactly that variant.
"""
...
async def test_inbox_dry_run_remediate_and_undo(tui, seed, fake_cloud):
    ...
    posts = [r for r in fake_cloud.requests() if r["method"] == "POST"
             and r["path"].startswith(f"/api/v1/findings/{flood}/")]
