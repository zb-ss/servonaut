...
before Memory Sync was set up, the user opens Memory Sync and unlocks it
with the passphrase from ``SERVONAUT_MEMORY_PASSPHRASE``: a keypair is made
on the device, its private half wrapped with the passphrase, and enrolled.
"Sync all local memory" backfills the probed modules and drains them to the
service as envelopes it cannot read: no probed value crosses the wire in
the clear, yet the key the client enrolled (unwrapped with the passphrase)
opens every envelope. After a re-probe and a second sync the service
...
async def test_restart_unlocks_from_the_local_key_cache(tui, seed, fake_cloud, monkeypatch):
    ...
    monkeypatch.setenv(PASSPHRASE_ENV, WRONG_PASSPHRASE)
    async with tui() as t:
        await open_memory_sync(t)
        assert status(t) == "⚪ Locked"
