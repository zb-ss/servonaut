...
from scripts.distribution.webview2_detect import WEBVIEW2_CLIENT_GUID
...
def _sanitize_wix_id(text: str) -> str:
    """Convert a path or string into a valid WiX identifier."""
    cleaned = re.sub(r"[^A-Za-z0-9_.]", "_", text)
    if cleaned and (cleaned[0].isdigit() or cleaned[0] == "."):
        cleaned = "_" + cleaned
    return cleaned[:70]  # WiX identifiers are limited in length
...
def _harvest_directory(
    ...
    for file_path in files:
        rel_path = file_path.relative_to(root_payload_dir)
        rel_str = str(rel_path).replace("\\", "/")
        guid = deterministic_guid(rel_str)
        safe_name = _sanitize_wix_id(rel_str)
        comp_id = f"C_{safe_name}"
        file_id = f"F_{safe_name}"

        # DirectoryRef to bind component to its directory
        target_dir_ref = "INSTALLFOLDER" if current_dir == root_payload_dir else dir_id_prefix

        comp_xml = (
            f'    <DirectoryRef Id="{target_dir_ref}">\n'
    ...
    for subdir in subdirs:
        rel_subdir = subdir.relative_to(root_payload_dir)
        sub_dir_id = f"Dir_{_sanitize_wix_id(str(rel_subdir))}"

        sub_nested_dirs, sub_comps, sub_refs, sub_count = _harvest_directory(
            root_payload_dir,
            subdir,
            dir_id_prefix=sub_dir_id,
        )
        ...
        dir_xml = (
            f'          <Directory Id="{sub_dir_id}" Name="{subdir.name}">\n'
            f'{sub_nested_dirs}'
            f'          </Directory>'
        )
...
def generate_wix_sources(
    ...
    *,
    ...
    # Validate XML syntax of generated product WXS
    try:
        ET.fromstring(wxs_content)
...
def package_windows(
    ...
    *,
    ...
    If WiX tools are installed on the host, compiles and links a standard .msi package.
    Otherwise (e.g. cross-platform or dry-run), produces validated WiX source files and
    deterministic container archive for testing and CI validation.
    ...
    light_bin = None
    if wix_bin_dir:
        cand_candidate = Path(wix_bin_dir) / ("candle.exe" if sys.platform == "win32" else "candle")
        light_candidate = Path(wix_bin_dir) / ("light.exe" if sys.platform == "win32" else "light")
        if cand_candidate.is_file() and light_candidate.is_file():
            candle_bin = str(cand_candidate)
            light_bin = str(light_candidate)
    else:
        candle_bin = shutil.which("candle.exe") or shutil.which("candle")
        light_bin = shutil.which("light.exe") or shutil.which("light")

    has_wix = bool(candle_bin and light_bin)

    if has_wix and not dry_run:
        # Run candle
        obj_path = wix_src_dir / "servonaut.wixobj"
        cmd_candle = [
            candle_bin,
            "-arch",
            "x64",
        ...
        # Fallback / cross-platform / dry-run: produce deterministic packaged MSI artifact
        # In this mode, we pack the generated WiX sources and payload manifest into a deterministic binary container
        # with standard OLE/MSI magic header simulation to enable full cross-platform qualification and testing.
        ...
        # Write simulated deterministic package container
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Generate WiX sources and validate layout without invoking WiX tools.")
    ...
    try:
        ...
        print(f"Windows MSI package created successfully: {result.msi_path}")
