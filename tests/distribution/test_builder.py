...
class TestManifestBuilder:
    def test_init_validation(self) -> None:
        builder = ManifestBuilder("2.27.0", channel=ReleaseChannel.STABLE, packaging_revision=1)
        ...
        with pytest.raises(ManifestSchemaError, match="not a valid Semantic Version"):
            ManifestBuilder("invalid-semver")

        # Negative revision
        with pytest.raises(ManifestSchemaError, match="non-negative integer"):
            ManifestBuilder("2.27.0", packaging_revision=-1)
    ...
    def test_build_signed_and_verify(self, tmp_path: Path) -> None:
        builder = ManifestBuilder("2.27.0", packaging_revision=2)
